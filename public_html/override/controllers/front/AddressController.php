...
Class AddressController extends AddressControllerCore
{
    ...
    public function getComunas(){
        ...
        foreach ($comunas as $comuna) {
            $selected = ($this->_address && ((int)$comuna['id_comunas'] === $this->_address->comuna)) ? ' selected="selected"' : '';
            $list .= '<option value="'.(int)$comuna['id_comunas'].'"'.$selected.'>'.htmlentities($comuna['nombre'], ENT_COMPAT, 'UTF-8').'</option>';
        }
 	...
 	protected function processSubmitAddress()
	{
	    ...
	    if ($this->context->customer->isLogged() && !$this->isTokenValid())
	 	...
	 	if (!Tools::getValue('comuna'))
    	$this->errors[] = Tools::displayError('The comuna is mandatory!');
	    // Check phone
