...
class ParentOrderControllerCore extends FrontController
{
    ...
    public function correctComuna($obj, $comunas){
        
        foreach ($obj as $key => $value) {
            $obj[$key]['formated']['comuna'] = $comunas[$obj[$key]['formated']['comuna']];
            $obj[$key]['object']['comuna'] = $comunas[$obj[$key]['object']['comuna']];
        }
        return $obj;
    }
    protected function _assignSummaryInformations()
    {
        ...
        $summary['formattedAddresses'] = $this->correctComuna($summary['formattedAddresses'],$comunas);
    ...
    protected function _assignAddress()
    {
        ...
        if (Validate::isLoadedObject($customer)) {
            ...
            $comunas_list = $this->getComunas();
            foreach ($customerAddresses as $key => $value) {
                $customerAddresses[$key]['comuna'] = $comunas_list[$customerAddresses[$key]['comuna']];
            }
            foreach ($formatedAddressFieldsValuesList as $key => $value) {
                $formatedAddressFieldsValuesList[$key]['formated_fields_values']['comuna'] = $comunas_list[$formatedAddressFieldsValuesList[$key]['formated_fields_values']['comuna']];
            }
            $this->context->smarty->assign(array(
    ...
    protected function _assignCarrier()
    {
        ...
        if(isset($address->comuna)){
            $comunas = $this->getComunas();
            $sql = new DbQuery();
            $sql->select('*');
            $sql->from('shipit_table', 'c');
            $sql->where('c.COMUNA = '.$address->comuna);
            $shipit_output = Db::getInstance()->executeS($sql);
            $shipit_output = $shipit_output[0];
            $datos_envio = array(
                'comuna' => $comunas[$address->comuna],
                'delay' => $shipit_output['PLAZO']
            );  
        }
        $this->context->smarty->assign(array(
            'address_collection' => $this->context->cart->getAddressCollection(),
            'shipping_data' => $datos_envio,
